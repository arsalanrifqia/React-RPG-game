...
export const CharacterCreator: React.FC<CharacterCreatorProps> = ({ uid, defaultName, onCharacterCreated }) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const newChar: CharacterData = {
      ...
      inventory: [
        { id: 'starter_sword', name: 'Rusty Blade', type: 'weapon', rarity: 'Common', stats: { strength: 3 }, value: 15, description: 'A rusted starter weapon.' },
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs bg-slate-900 p-2.5 rounded-xl border border-slate-800">
                      <div><span className="text-slate-500">HP:</span> {info.baseStats.hp}</div>
                      <div><span className="text-slate-500">Mana:</span> {info.baseStats.mana}</div>
                      <div><span className="text-slate-500">STR:</span> {info.baseStats.strength}</div>
                      <div><span className="text-slate-500">INT:</span> {info.baseStats.intelligence}</div>
                      <div><span className="text-slate-500">AGI:</span> {info.baseStats.agility}</div>
                      <div><span className="text-slate-500">VIT:</span> {info.baseStats.vitality}</div>
                    </div>
                  </div>
                );
              })}
              ...
              {activeClassData.skills.map((skill) => (
                <div key={skill.id} className="bg-slate-900 border border-slate-800 rounded-xl p-3">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-xs text-indigo-400 bg-indigo-950/80 px-2 py-0.5 rounded-full border border-indigo-800">
                      {skill.manaCost} MP
                    </span>
                  </div>
